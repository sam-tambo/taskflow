      ...
      <div className="flex items-center gap-3 px-4 py-2 border-b border-gray-200 dark:border-slate-800">
        ...
        <div className="ml-auto flex gap-1">
          {(['days', 'weeks', 'months'] as Zoom[]).map((z) => (
            <button key={z} onClick={() => setZoom(z)} className={cn('px-2 py-1 text-xs rounded', zoom === z ? 'bg-coral text-white' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-slate-800')}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-64 flex-shrink-0 border-r border-gray-200 dark:border-slate-800 overflow-y-auto">
          {tasksWithDates.map((task) => (
            <div key={task.id} onClick={() => openTaskDetail(task.id)} className="flex items-center gap-2 px-3 py-2 border-b border-gray-100 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-slate-800/50 cursor-pointer text-sm">
              {task.is_milestone ? (
                <Diamond className="w-3.5 h-3.5 text-coral flex-shrink-0" />
              ) : (
        ...
        <div ref={containerRef} className="flex-1 overflow-auto relative">
          ...
          <div className="sticky top-0 z-10 flex bg-gray-50 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-800" style={{ width: totalWidth }}>
            {days.map((day, i) => (
              <div key={i} className={cn('flex-shrink-0 text-center text-xs py-2 border-r border-gray-100 dark:border-slate-800', isToday(day) && 'bg-coral/10 font-semibold text-coral')} style={{ width: dayWidth }}>
              ...
              return (
                <div key={task.id} className="relative" style={{ height: 36 }}>
                  {task.is_milestone ? (
                    ...
                    >
                      <Diamond className="w-4 h-4 text-coral fill-coral" />
                    </div>
                  ) : (
                    <div
                      onClick={() => openTaskDetail(task.id)}
                      className={cn('absolute top-1.5 h-6 rounded-full cursor-pointer transition-opacity hover:opacity-80', task.status === 'done' ? 'bg-green-400' : 'bg-coral')}

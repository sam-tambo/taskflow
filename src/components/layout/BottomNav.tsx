    ...
    <>
              ...
              {moreItems.map(({ path, label, icon: Icon }) => (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive(path) ? 'bg-coral/10 text-coral' : 'text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'
                  )}
                >
      ...
      <nav className="fixed bottom-0 left-0 right-0 md:hidden bg-white dark:bg-slate-900 border-t border-gray-200 dark:border-slate-800 z-30" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
        <div className="flex items-center justify-around h-14">
          <Link to="/" className={cn('flex flex-col items-center gap-0.5 px-3 py-1', isActive('/') ? 'text-coral' : 'text-gray-500 dark:text-slate-400')}>
            <Home className="w-5 h-5" />
            <span className="text-[10px] font-medium">Home</span>
          </Link>
          <Link to="/inbox" className={cn('relative flex flex-col items-center gap-0.5 px-3 py-1', isActive('/inbox') ? 'text-coral' : 'text-gray-500 dark:text-slate-400')}>
          ...
          <button
            ...
            className={cn('flex flex-col items-center gap-0.5 px-3 py-1', showMore ? 'text-coral' : 'text-gray-500 dark:text-slate-400')}
          >
